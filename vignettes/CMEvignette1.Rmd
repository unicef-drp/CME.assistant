...
head(dt_2)
# With a given `dir_file`, can also use `read.country.summary`, which is a simplier version that only returns long-format data
dt_3 <- read.country.summary(dir_file = dir_list$dir_female_2020)

# load all in one step
dt_all <- data.table::rbindlist(lapply(load.final_dir(2020), get.CME.UI.data, get = "both"), fill = TRUE)
dt_all[, table(Sex, Shortind)]
```
...
dir_U5MR   <- get.dir_U5MR(input_dirs$dir_IGME_21)
dir_IMR    <- get.dir_IMR(input_dirs$dir_IGME_21)
